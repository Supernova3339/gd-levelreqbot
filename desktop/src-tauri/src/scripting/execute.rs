...
//
// Performance:
//   - stdlib ASTs are compiled once and reused (see stdlib.rs)
//   - per-command script ASTs are cached by source hash
//   - Engine is created per-call but contains only type registrations (fast)
...
fn compile_or_cached(engine: &rhai::Engine, src: &str) -> AST {
...
async fn run_script_inner(
    ...
    let result = tokio::task::block_in_place(|| {
        ...
        // ── Load stdlib (uses pre-compiled ASTs) ───────────────────────────
        load_stdlib(&engine, &mut scope);

        // ── Compile (cached by source hash) ───────────────────────────────
        let ast = compile_or_cached(&engine, &script_owned);

        // For test mode: also attempt compile to get user-visible error messages
        if capture_err {
            if let Err(e) = engine.compile(&script_owned) {
                errors_ref.lock().unwrap().push(format!("Compile error: {e}"));
                return Ok(());
            }
        }
